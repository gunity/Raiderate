...
{
    public async Task<RatingReason?> GetAsync(Guid id, CancellationToken ct = default)
    {
        return await context.RatingReasons
            .FirstOrDefaultAsync(x => x.Id == id, ct);
    }

    public async Task<RatingReason?> GetActiveReadonlyAsync(Guid id, CancellationToken ct = default)
    {
        return await context.RatingReasons
            .AsNoTracking()
